...
while datetime.now() < end_time:
    ...
    if 'VTG' in nmea_sentence:
        ...
        try:
            kmh = float(msg_kmh.spd_over_grnd_kmph)
        except:
            kmh = 0.0

    if 'GGA' in nmea_sentence:
        msg_pos = pynmea2.parse(nmea_sentence)
        try:
            lat = float(msg_pos.latitude)
            lon = float(msg_pos.longitude)
        except:
            lat = 0.0
            lon = 0.0
    OwlBox_BSM["lat"] = lat
    OwlBox_BSM["lon"] = lon
    OwlBox_BSM["speed"] = kmh
